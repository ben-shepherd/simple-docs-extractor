...
export type DocumentContentExtractorConfig = ExtractionMethod | ExtractionMethod[];
...
export class DocumentContentExtractor {
    ...
    async extractFromString(contents: string): Promise<ExtractionResult[]> {
        const extractionMethodsArray = Array.isArray(this.config) ? this.config : [this.config];
     ...
     */
    private async handleExtractionMethod(method: ExtractionMethod, fileContent: string, i: string, results: ExtractionResult[]) {
        ...
        else if (method.extractMethod === 'callback' && typeof method.callback === 'function') {
    ...
    private trimContent(result: ExtractionResult) {
        const trimCallback = (content: string) => content.trim().replace(/\n\s*\n/g, '\n');
     ...
     */
    protected extractUsingRegex(method: MethodRegex, fileContent: string): ExtractionResult | ErrorResult {
     ...
     */
    protected async extractUsingCallback(method: MethodCallback, fileContent: string): Promise<ExtractionResult | ErrorResult> {
     ...
     */
    protected extractUsingTags(method: MethodTags, fileContent: string): ExtractionResult | ErrorResult {
        // Check if the file contains the start and end tags
        if (!fileContent.includes(method.startTag) || !fileContent.includes(method.endTag)) {
            return {
        ...
        */
        ...
        const finalRegex = new RegExp(`${startTagPattern}(${inBetweenTagsPattern}*?)${endTagPattern}`, flags);

        // match the final regex
        const matches = fileContent.match(finalRegex);

        // Remove the start and end tags from each match
        const matchesWithoutTags = matches?.map(match => match?.replace(method.startTag, '').replace(method.endTag, ''));
