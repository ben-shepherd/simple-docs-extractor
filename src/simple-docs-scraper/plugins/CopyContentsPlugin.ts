 ...
 * </docs>
 */
export class CopyContentsPlugin
  implements ExtractorPlugin<CopyContentsPluginConfig>
{
  ...
  async extractFromString(): Promise<ExtractedContent[] | ErrorResult> {
    const fileToCopy = Array.isArray(this.config.fileToCopy)
      ? this.config.fileToCopy
      : [this.config.fileToCopy];
