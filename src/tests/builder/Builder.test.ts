...
describe("Builder", () => {
  ...
  describe("config", () => {
    test("should be able to configure the builder", () => {
      const config = SimpleDocExtractor.create(process.cwd())
      ...
      expect(config.templates?.index?.templatePath).toBe(
    ...
    test("should be able to define templates in the target", () => {
      ...
      expect(config.targets[0].templates?.index?.templatePath).toBe(
        "src/templates/index.template.md",
      );
      ...
      expect(config.targets[0].templates?.rootIndex?.templatePath).toBe(
        "src/templates/root-index.template.md",
      );
      ...
      expect(config.targets[0].templates?.rootIndex?.excerpt?.addEllipsis).toBe(
        false,
      );
      expect(
        config.targets[0].templates?.rootIndex?.excerpt?.firstSentenceOnly,
      ).toBe(true);
    });
  });
});
