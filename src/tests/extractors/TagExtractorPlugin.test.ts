...
describe("Tag Extractor", () => {
  const sampleText = `<docs>This is a test</docs>`;

  const sampleTextMultiLine = `<docs>
This is a test
</docs>`;
 ...
 * </docs>
 */`;
  const sampleTextMultipleTags = `<docs>
This is the first block
</docs>
<docs>
This is the second block
</docs>`;
  const sampleTextWithAttributes = `<docs name="nameValue" other="otherValue">
This is the first block
</docs>`;
  const sampleTextWithAttributesMultipleTags = `<docs name="nameValue" other="otherValue">
This is the first block
</docs>
<docs>
This is the second block
  ...
  describe("methods", () => {
    ...
    test("should get start tag pattern", () => {
      ...
      const result3 =
        '<docs name="nameValue" other="otherValue" with_underscore="with_underscoreValue">'.match(
          regExp3,
        );
      expect(result3?.[0]).toBe(
        '<docs name="nameValue" other="otherValue" with_underscore="with_underscoreValue">',
      );
    });
    ...
    test("should extract attributes", () => {
      ...
      const test =
        'name="nameValue" other="otherValue" with_underscore="with_underscoreValue"';
    ...
    test("should get inside tag pattern", () => {
      ...
      const multiLineTextWithAttributes = `This is the first line
...
This is the third line`;
      ...
      expect(result3?.input).toContain(
  ...
  describe("main extractUsingTags logic", () => {
    test("should extract content from main method as expected from sampleText", async () => {
      const result = (await tagExtractor.extractFromString(
        sampleText,
    ...
    test("should be able to extract multiple tags, both with and without attributes", async () => {
      const result = (await tagExtractor.extractFromString(
        sampleTextWithAttributesMultipleTags,
      )) as unknown as ExtractedContent[];
