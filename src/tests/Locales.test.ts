...
describe("Locales", () => {
  ...
  describe("getLocales", () => {
    ...
    describe("getLocalesAsExtractedContents", () => {
      test("should perform expected behavior", () => {
        const extractedContentArray = new LocalesService(
          getOutputPath("test.txt"),
        ).getLocalesAsExtractedContents();

        const updatedAt = extractedContentArray.find(
          (content) => content.searchAndReplace === "%locales.updatedAt%",
        );
        const fileName = extractedContentArray.find(
          (content) => content.searchAndReplace === "%locales.fileName%",
        );
